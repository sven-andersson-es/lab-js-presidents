...
function getNames(presidentsArr) {
	const namesOfPresidents = presidentsArr.map((president) => {
...
function getDemocraticPresidents(presidentsArr) {
	const bluePresidents = presidentsArr.filter((president) => {
		return president.party === "Democratic";
	});
	return bluePresidents;
}
...
function countYearsInOffice(presidentsArr) {
	let yearsInOffice = presidentsArr.reduce((acc, cur) => {
...
// countYearsInOffice(testPresidents);

// Iteration 4 | Sort Presidents by Birth Year - `sort()`
function sortPresidentsByBirthYear(presidentsArr) {
	const presidentsByBirthYear = presidentsArr.sort((a, b) => {
...
function getAgeAtInauguration(presidentsArr) {
  const inaugurationAgeAdded = presidentsArr.map((president) => {
    const ageAtInauguration = president.tookOffice - president.birthYear;
    return {...president, ageAtInauguration: ageAtInauguration}
  })
return inaugurationAgeAdded;

}
console.log(getAgeAtInauguration(testPresidents))
...
function sortPresidentsByName(presidentsArr) {
  const presidentsSortedByName = presidentsArr.sort((a,b) => {
    return a.name.localeCompare(b.name, 'en', {sensitivity: 'base'})
  });
  return presidentsSortedByName;
}
